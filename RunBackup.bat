...
echo     $exePath = "C:\Program Files\FreeFileSync\RemoveDrive.exe" >> "%PS_SCRIPT%"
echo. >> "%PS_SCRIPT%"
echo     if (Test-Path $exePath) { >> "%PS_SCRIPT%"
echo         # The -L argument tells RemoveDrive to loop and try again if the drive is temporarily busy >> "%PS_SCRIPT%"
echo         $process = Start-Process -FilePath $exePath -ArgumentList "Z: -L" -NoNewWindow -Wait -PassThru >> "%PS_SCRIPT%"
echo. >> "%PS_SCRIPT%"
echo         # ExitCode 0 means it successfully safely removed the hardware >> "%PS_SCRIPT%"
echo         if ($process.ExitCode -eq 0) { >> "%PS_SCRIPT%"
echo             [System.Windows.Forms.MessageBox]::Show("Drive Z: was successfully and safely disconnected. You can now unplug it.", "Eject Complete", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) >> "%PS_SCRIPT%"
echo         } else { >> "%PS_SCRIPT%"
echo             [System.Windows.Forms.MessageBox]::Show("RemoveDrive could not eject Z:. Make sure no files are open.", "Eject Failed", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning) >> "%PS_SCRIPT%"
echo         } >> "%PS_SCRIPT%"
echo     } else { >> "%PS_SCRIPT%"
echo         [System.Windows.Forms.MessageBox]::Show("Could not find RemoveDrive.exe at $exePath", "Missing File", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error) >> "%PS_SCRIPT%"
echo     } >> "%PS_SCRIPT%"
echo. >> "%PS_SCRIPT%"
